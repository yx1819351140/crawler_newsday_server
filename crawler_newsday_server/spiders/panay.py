...
class PanaySpider(scrapy.Spider):
    name = "panay"
    # allowed_domains = ["panaynews.net"]
    # start_urls = ["https://panaynews.net"]
    mongo = MongoDB()
    logger = logging.getLogger(__name__)
    settings = get_project_settings()
    category_list = ['news', 'opinion', 'sports', 'business', 'lifeandstyle', 'entertainment', 'world', 'nation']

    def start_requests(self):
        for category in self.category_list:
            for page in range(1, 6):
                url = f'https://www.panaynews.net/category/{category}/page/{page}/'
                yield Request(url, callback=self.parse, meta={'category': category})

    def parse(self, response):
        try:
            url_list = response.xpath('//h3[@class="entry-title td-module-title"]/a/@href').getall()
            for url in url_list:
                try:
                    _id = md5_string(url)
                    ...
                    if is_repeat:
                        ...
                        continue
                    yield Request(url + '?amp=1#', callback=self.parse_detail, meta={'category': response.meta.get('category')})
                except:
                    continue
    ...
    def parse_detail(self, response):
        try:
            ...
            try:
                ...
                content_html_list = html.xpath('//div[@class="td-post-content tagdiv-type"]/p')
            ...
            if title and content:
                ...
                try:
                    ...
                    publish_timestamp = ''
                try:
                    author_name = response.xpath('//span[@itemprop="author"]/meta[@itemprop="name"]/@content').get()
                except:
                    author_name = ''
                try:
