...
class ManilatimesSpider(scrapy.Spider):
    ...
    # start_urls = ["https://manilatimes.net"]
    mongo = MongoDB()
    logger = logging.getLogger(__name__)
    settings = get_project_settings()
    category_list = ['opinion', 'news', 'regions', 'world', 'business', 'sports', 'entertainment-lifestyle/show-times',
                     'entertainment-lifestyle/life-times', 'the-sunday-times', 'supplements', 'special-sections',
                     'public-square', 'legal-advice', 'fast-times', 'campus-press', 'expats-diplomats', 'podcasts']
    ...
    def parse(self, response):
        try:
            url_list = response.xpath('//div[@class="content-row flex-row"]//div[@class="article-title-h4 ellipsis-2 mr-b10"]/a/@href').getall()
            for url in url_list:
                _id = md5_string(url)
    ...
    def parse_detail(self, response):
        try:
            ...
            try:
                ...
                content_html_list = html.xpath('//div[@class="article-body-content"]/p')
